...
const MessageQueue = () => {
  ...
  const handleAction = async (id: string, action: 'approve' | 'skip') => {
    const newStatus = action === 'approve' ? 'scheduled' : 'skipped';
    const { error } = await supabase
      .from('messages')
      .update({ status: newStatus })
      .eq('id', id);
    if (error) { toast.error('Failed to update message'); return; }
